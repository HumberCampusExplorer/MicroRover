//translate([0,0,-3]){import("A:\\2017jan-apr\\general\\summer\\OringServoPulleyK.stl", convexity=3);}
//---------------------------------------------------------------------------------
...
translate([0,0,wheel_height]){mirror([0,0,1]){main();}}
...
module main() {
	difference() {
		...
		// Put notch in rim.
		translate([0,0, wheel_height/2]) // Center along rim
		rotate_extrude($fn=100)
		//polygon([[inner_radius,notch_height/2],[wheel_radius,notch_height],[wheel_radius+1,-2]]);
	polygon([[wheel_radius+.1,2],[wheel_radius+.1-notch_height,0],[wheel_radius+.1,-2]]);
	
		// Subtract servo mount.
		servomount(counter_sink_height=wheel_height+.1, counter_sink_diameter=7.0);
...
module servomount(counter_sink_height=1.5, counter_sink_diameter=6.0, num_teeth=21) {
	spline_height = 2.5; //3.8
	outer_diameter = 5.1; //This is what I increased from v1 to v2. Was 6.0, now 6.1. Hopefully it'll not be such a tight press fit.
	inner_diameter = 4.6;
	screw_diameter = 2.2;
